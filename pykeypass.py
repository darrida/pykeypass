...
def path_selection(test=False):
    ...
    """
    pykeypass_folder = (
        Path.home() / ".pykeypass" if test is False else Path.cwd() / "test" / ".pykeypass"
    )
...
def keepass_open(database, test, input_password=None):
    ...
    try:
        pykeypass_folder, pykeypass_app, pykeypass_db = path_selection(test)
        password = (
            getpass.getpass("pykeepass password: ") if input_password is None else input_password
        )
        ...
        if str(e) == "'NoneType' object has no attribute 'url'":
            ...
            cecho(
                ...
                'this entry using: "pykeypass open {database} -s"'
            )
        else:
